...
namespace Cubbit
{
	...
	void Horcrux::Encrypt(char* inputFile, int& chunks, char* outputDirectory, char* fileNamePrefix)
	{
		// Set a file name prefix
		if (!fileNamePrefix)
			fileNamePrefix = (char*) (std::string(inputFile).substr(std::string(inputFile).find_last_of("/\\") + 1).c_str());
